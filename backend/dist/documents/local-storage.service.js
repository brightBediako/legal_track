...
let LocalStorageService = class LocalStorageService {
    constructor() {
        this.uploadRoot = path.resolve(__dirname, '..', '..', 'upload');
    }
    getUploadRoot() {
    ...
    async storeLocalFile(sourcePath, originalName) {
    ...
    resolveAbsolutePath(key) {
        ...
        if (resolved !== this.uploadRoot && !resolved.startsWith(rootWithSep)) {
            throw new common_1.BadRequestException('invalid local file key');
        }
        return resolved;
    }
    async getAbsolutePath(key) {
        ...
        try {
            ...
            throw new common_1.NotFoundException('local file not found on disk');
        }
        return absolutePath;
    }
};
